		...
		return (
			<Swipeable
				...
				onRightActionRelease={() => {
					this.onDeleteEvent(this.props.event);
				}}>
				<View style={eventListStyles.container}>
					<View
						style={eventListStyles.color}
						backgroundColor={this.props.event.typeRef.color}></View>
					<Image
						style={eventListStyles.icon}
						source={this.props.event.typeRef.icon}></Image>
...
class CalendarPage extends Component {
	...
	onCalendarDateSelected = date => {
		this.setState({
			selectedDate: moment(date.dateString, 'YYYY-MM-DD').toDate(),
		});
	};
	...
	onNewEventDoneBtnClicked() {
		const newEvent = new Event({
			index: this.state.newEventIndex,
			comment: this.state.newEventLabel,
	...
	render() {
		...
		EventTypes.all.map((eventType, index) => {
			eventTypePickerData.push({
				label: eventType.title,
				value: eventType.title,
			});
		});
		const events = this.props.events ? this.props.events : [];
		const currentEventList = EventManager.eventWithDate(
			events,
				...
				<View style={{ flex: 1 }}>
					<ScrollView>
						...
						<Text style={styles.description2}>
							{"You'll get a Ping on the day of the event."}
						</Text>
						<Calendar
							ref={calendar => {
						...
						<TouchableOpacity
							...
							<Image
								style={styles.calendarAdd}
								source={require('../../assets/calendar_add.png')}></Image>
						</TouchableOpacity>
						...
						<View
							...
							<View style={modalStyles.container}>
								...
								<View style={modalStyles.eventTypeContainer}>
									<Image
										style={modalStyles.eventTypeIcon}
